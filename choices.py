...
class MatchUp(object):
	...
	def one(self):
		fields = [str(self.date), self.sport, self.prompt, self.team1, str(self.winner1), str(self.result1), str(self.percent1), str(self.percent_active)]
		return fields
	def two(self):
		fields = [str(self.date), self.sport, self.prompt, self.team2, str(self.winner2), str(self.result2), str(self.percent2), str(self.percent_active)]
		return fields
######################################
...
def getMatchups(date, writer=None):
	...
	for i in matchups:
		...
		m.append(ob)
		if writer != None:
			writer.writerow(ob.one())
			writer.writerow(ob.two())
